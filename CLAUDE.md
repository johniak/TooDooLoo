...
- **Kolor tożsamości zadania**: `taskColor(todo)` w core.ts — ręczny `todo.color`, a bez niego hash z `id` na 8-kolorową paletę `TASK_COLORS` (żywe pastele: malina→słoma; **bez brązów i pomarańczy — ta strefa należy do embera**). Lewa krawędź karty (3px), ta sama na duchu i blokach osi czasu. Zmiana: przycisk z kropką koloru na wierszu todosa → picker swatchy + „↺ Automatyczny" (pusty string w `updateTodo` kasuje pole); w MCP `update_todo.color`. Pilność (ember-dot) zostaje osobnym wymiarem koloru.
...
- **Notatka ticketa**: każdy todos ma lazy-tworzoną notatkę `notes/todo-<todoId>.md` (tytuł = tekst todosa, synchronizowany przy `updateTodo.text`; `saveTodoNote` w store). UI: przycisk ✎ na wierszu todosa (podświetlony dawn, gdy notatka istnieje) rozwija inline edytor pod wierszem (`TodoNote.tsx`, wzór DayNote: debounce + flush). W eksploratorze wirtualny katalog „⌗ Notatki ticketów" (domyślnie zwinięty, klik otwiera zwykły edytor). `todo-*` wykluczone z realNotes/badge'y/pickerów jak `day-*`. MCP: `get/set_todo_note`.
...
- Eksplorator notatek: pozycja „▤ Notatki" na dole sidebara — osobny widok panelu (`view: 'day' | 'notes' | 'settings' | 'timeline' | 'reports'` w App) z **drzewem notatek**: podstrony wcięte pod rodzicami (rekurencyjny `TreeRows`, trójkąciki ▸/▾, zwijanie w stanie komponentu), na końcu wirtualny katalog „▦ Notatki dni" (domyślnie zwinięty; klik w dzień → `onOpenDay` przełącza na widok tego dnia, bo notatki dni edytuje się inline). Link z todosa przełącza na ten widok. Badge'y sidebara liczą tylko zwykłe notatki. **Drag & drop**: wiersz drzewa jest draggable (HTML5 DnD, `text/note-id` w dataTransfer) — upuszczenie na inną notatkę wpina jako podstronę, na nagłówek „Wszystkie notatki" — odpina na top-level; `saveNote` przyjmuje `parentId` ('' odpina) i **odrzuca cykle** (`inSubtree` w store — obowiązuje też MCP `update_note.parentId`). Nowa notatka z `[[...]]` (klik w nieistniejącą albo „＋ Utwórz" w pickerze) powstaje jako **podstrona notatki-gospodarza** (prop `noteId` w VisualEditor → `parentId` w evencie/creatcie); z notatki dnia — na top-level.
- **Raport** („▥ Raport", `Reports.tsx`): tygodniowy raport à la Timemator Reports, liczony w całości z `weekBlocks`. Trzy części: **słupki dzienne** (stack segmentów per zadanie w kolorach tożsamości, stały porządek stosu po todoId, suma `h:mm` nad słupkiem, gridlines co 30 min/1h/4h zależnie od maksimum, wszystkie 7 dni, dzisiaj ember), **legenda zadań** (kolor + suma tygodnia, sort malejąco, „Razem" na dole), **log sesji** (dni malejąco, wiersze: koniec/start w mono — sesja w toku ma kropkę zamiast końca, krawędź koloru, czas trwania; klik → widok dnia todosa). Ta sama nawigacja co oś czasu (helpery `weekDates`/`shortDate` eksportowane z Timeline.tsx). **Zakres**: przełącznik Tydzień/Miesiąc (pigułki w nagłówku; miesiąc = słupek per dzień, wąskie, etykiety tylko poniedziałki i 1., bez sum nad słupkami — tooltip na słupku; tytuł zakresu z `Intl.DateTimeFormat pl-PL`; zmiana zakresu zeruje offset). **Raport per zadanie**: klik w zadanie w legendzie filtruje słupki/log/sumy do niego (aria-pressed + emberowy inset, „✕ Wszystkie zadania" zdejmuje, drugi klik też); legenda zawsze liczona z pełnego zakresu.
- **Oś czasu** („▦ Oś czasu" w stopce sidebara, `Timeline.tsx`): tygodniowy kalendarz sesji trackingu à la Timemator. `weekBlocks(todos, dates, now)` w core.ts tnie sesje granicami dób i układa **nakładki klasycznym algorytmem kalendarzowym** (klastry przecinających się bloków → sloty `lane/lanes`, szerokość bloku = 1/n klastra); sesja otwarta = blok do „teraz" z flagą `running` (emberowa poświata). Blok: przezroczysty fill `taskColor` + 3px lewa krawędź + tytuł w kolorze + czas trwania; tekst znika poniżej ~24 min (zostaje tooltip). Oś godzin adaptacyjna (workStart−1h → workEnd+2h, rozciągana przez sesje), kolumny Pn–Pt + weekend gdy ma bloki, emberowa linia „teraz" w dzisiejszej kolumnie, suma tygodnia w nagłówku, nawigacja ‹ ● ›. **Edycja sesji**: klik w blok → modal (start data+czas, koniec data+czas, długość w min; link „▤ Pokaż todosa"; „Usuń" kasuje zalogowany czas — tylko dla zamkniętej sesji, `store.deleteSession` odmawia dla otwartej); bloki można **przeciągać** (pion = czas, poziom = zmiana dnia) i **rozciągać** za górną/dolną krawędź (strefa 7px z kursorem `ns-resize` — spany `.tl-handle`; tylko na kawałku z prawdziwym startem/końcem — `isStart`/`isEnd` na bloku). Snap **absolutny do siatki 5 min** (`dragPatch` snapuje wynik, nie deltę; przesuwanie zachowuje długość co do sekundy). Sesja otwarta: koniec nietykalny (modal pokazuje „● w toku", drag tylko za górę); reguły w `store.updateSession` (IPC `session:update`), walidacja start<koniec. Podgląd dragu liczony lokalnie (przesunięta kopia sesji → `weekBlocks`), commit na pointerup. Sesja w toku: pulsująca kropka `tl-live-dot` przy tytule (kolor tożsamości zostaje), proste dolne rogi — blok kończy się równo na linii „teraz" (linia bez kropki, z-index nad blokami). Bez zoomu i widoku dziennego. IPC `todos:all`; komponent sam się karmi (`listAllTodos` + `onDataChanged`, tick 60 s).
- **Auto-linki w notatkach** (`reflinks.ts`, rozszerzenie TipTap z dekoracjami ProseMirror — markdown na dysku zostaje czystym tekstem): `#N` → skok do todosa o tym `num` (klik → event `open-todo-num` na window, App nasłuchuje i podświetla), `AB#123` → Azure Boards, `GH#123` → GitHub `/issues/N` (przekierowuje też na PR-y), `[[Tytuł]]` → notatka po tytule (wiki-styl: klik w nieistniejącą **tworzy ją** — event `open-note-title`); wstawianie przez przycisk `▤+` w toolbarze edytora (picker: szukaj istniejących / „＋ Utwórz"). Bazowe URL-e w ustawieniach (`azureBase`, `githubBase`, sekcja „Linki w notatkach"); pusta baza = brak akcji. Lookbehind w regexie `#N` pomija środek `AB#`/`GH#`. Działa w trybie wizualnym (notatki + notatka dnia); surowy Md bez dekoracji. Settings w App to jeden obiekt (typ `Settings` z shared/store, import type w rendererze).
